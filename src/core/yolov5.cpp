...
void ai::YOLOV5::nms(std::vector<Object> &input, std::vector<Object> &output, float iou_threshold) 
{
...
void ai::YOLOV5::generate_anchors(int in_shape)
{
    for (size_t i = 0; i < strides.size(); i++)
    {
        ...
        std::vector<ai::YoloV5Anchor> anchors;
...
void ai::YOLOV5::read_yaml(const std::string& model_path)
{
...
void ai::YOLOV5::post_processing(
...
)
{
    ...
    for (size_t n = 0; n < vectors[1].size(); n++)
    {
        ...
        for (unsigned int i = 0; i < num_anchors; ++i)
        {
            ...
            if (!with_output)
                offset_obj_cls_ptr = (float *) output.vec.data() + (i * (num_classes + 5)); // row ptr;

            obj_conf = with_output ? output.vec.data()[i*output.h+4] : ai::sigmoid(offset_obj_cls_ptr[4]);
            cls_conf = with_output ? output.vec.data()[i*output.h+5] : ai::sigmoid(offset_obj_cls_ptr[5]);
            ...
            {
                float tmp_conf = with_output ? output.vec.data()[i*output.h + j + 5] : ai::sigmoid(offset_obj_cls_ptr[j + 5]);
            ...
            if (with_output)
            {
                dw = 0;
                dh = 0;
                cx = output.vec.data()[i*output.h + 0];
                cy = output.vec.data()[i*output.h + 1];
                w = output.vec.data()[i*output.h + 2];
                h = output.vec.data()[i*output.h + 3];
            }
            else
            {
...
void ai::YOLOV5::pre_processing(ai::Mat<unsigned char>& mat)
{
    // TODO letterbox
}

void ai::YOLOV5::predict(
